...
router.post('/',[auth,admin], async (req, res)=>{
    ...
    const customerData = await Customer.findById(new mongoose.Types.ObjectId(req.body.customerId));
    if(!customerData) return res.send('CustomerId not sent');

    const movieData = await Movie.findById(new mongoose.Types.ObjectId(req.body.movieId))
